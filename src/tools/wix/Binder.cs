...
namespace WixToolset
{
    ...
    public sealed class Binder
    {
        ...
        private void BindBundle(Output bundle, string bundleFile, out IEnumerable<FileTransfer> outFileTransfers, out IEnumerable<string> contentPaths)
        {
            outFileTransfers = Enumerable.Empty<FileTransfer>();
            contentPaths = Enumerable.Empty<string>();
            ...
            if (delayedFields.Any())
            {
                ...
                resolveDelayedFieldsCommand.OutputType = bundle.Type;
            ...
            outFileTransfers = fileTransfers;
            contentPaths = allPayloads.Values.Where(p => p.ContentFile).Select(p => p.FullFileName).ToList();
        }
